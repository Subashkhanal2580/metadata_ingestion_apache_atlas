...
def verify_atlas_metadata():
    ...
    try:
        ...
        # Use the correct parameters for our test table
        # Note: We're now looking for the 'employees' table in the 'atlas_test' database
        result = verify_metadata(
            atlas_url="http://atlas:21000/api/atlas/v2/search/basic",
            table_name="employees",  # Changed from 'transformed_data' to 'employees'
            database="atlas_test",   # Changed from 'default' to 'atlas_test'
...
wait_for_metadata = BashOperator(
    ...
    bash_command='sleep 90',  # Increased wait time for metadata to propagate to Atlas
    dag=dag,
)
